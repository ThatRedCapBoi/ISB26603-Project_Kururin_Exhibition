...
import 'package:firebase_auth/firebase_auth.dart' as auth; // ADDED 'as auth' HERE
...
class _LoginPageState extends State<LoginPage> {
  ...
  void _login() async {
    ...
    try {
      ...
      if (adminDoc.exists && adminDoc.data() != null && (adminDoc.data() as Map<String, dynamic>)['isAdmin'] == true) {
        // User is an admin
        final admin = Admin.fromFirestore(adminDoc); // This is your custom Admin model
        ScaffoldMessenger.of(context).showSnackBar(
          const SnackBar(content: Text('Admin login successful!')),
        );
        Navigator.pushReplacement(
          context,
          MaterialPageRoute(builder: (context) => AdminHomePage(admin: admin)), // Pass the 'admin' object here
        );
      } else {
        ...
        if (userDoc.exists && userDoc.data() != null) {
          final user = User.fromFirestore(userDoc); // This is your custom User model
          ScaffoldMessenger.of(context).showSnackBar(
            const SnackBar(content: Text('Login successful!')),
          );
          Navigator.pushReplacement(
            context,
            MaterialPageRoute(builder: (context) => UserHomePage(user: user)), // Pass the 'user' object here
          );
        } else {
          // This case should ideally not happen if registration worked
          ScaffoldMessenger.of(context).showSnackBar(
            const SnackBar(content: Text('User profile not found in database. Please register or contact support.')),
          );
          await auth.FirebaseAuth.instance.signOut(); // Use 'auth.FirebaseAuth' here
        }
      }
    } on auth.FirebaseAuthException catch (e) { // Use 'auth.FirebaseAuthException' here
